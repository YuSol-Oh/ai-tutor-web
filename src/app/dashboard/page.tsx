    ...
    <header className="shrink-0 border-b border-ink-200 bg-white/90 backdrop-blur z-10">
      <div className="flex items-center gap-3 px-8 h-16">
        ...
        <nav className="ml-6 hidden lg:flex items-center gap-1 text-[13px]">
          <Link href="/dashboard" className="px-3 h-9 inline-flex items-center rounded-lg text-ink-900 font-semibold bg-ink-100 whitespace-nowrap">대시보드</Link>
...
function Sidebar({ userName, userField, level, streak }: {
  ...
  const initial = userName.slice(-2, -1) || userName[0] || "U";
  const items = [
    { href: "/dashboard", label: "홈",          Icon: IHome,    badge: null,  active: true  },
...
export default async function DashboardPage() {
  ...
  if (!profile || !curriculum) redirect("/onboarding");

  const topics: TopicRow[] = Array.isArray(curriculum.topics) ? curriculum.topics as TopicRow[] : [];
  const currentIndex = (profile.current_topic_index as number) ?? 0;
  const totalTopics  = (curriculum.total_topics as number) ?? topics.length;
  ...
  const userName    = (profile.name as string | null) || user.email?.split("@")[0] || "학습자";
  const userField   = subjectMap[profile.subject as string] ?? (profile.subject as string);
  const level       = levelMap[profile.current_level as string] ?? (profile.current_level as string);
